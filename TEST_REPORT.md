...
> 2026-08-04 内网 ASR 试运行：在 `192.168.0.119:18766` 安装 faster-whisper 1.1.1、下载 base/small，并以独立 `voicescreen-asr.service` 常驻。16kHz PCM 英文整句识别准确，两次耗时约 7.1 秒和 7.3 秒；进程驻留约 700 MB，推理时接近占满 4 线程。18765 翻译/Piper 服务保持独立运行。

> 2026-08-04 本地服务隔离回归：发现旧版“自建模式”的仅 ASR 进程与完整本地 OPUS-MT 共用 18765，切换模式后可能误连并触发 tokenizer 为 `None`。现已固定拆分为完整本地服务 `127.0.0.1:18765`、仅 ASR 服务 `127.0.0.1:18766`，健康检查同时验证真实能力。清理遗留进程后完整本地双向、泰语桥接、低延迟字幕和长句测试全部通过。内网服务器公布的 Lessac、Joe、Mike、John 四个英文 Piper 音色均已完成真实 WAV 合成测试。

> 2026-08-04 增量验证：新增免费自建 OPUS-MT + Piper 客户端链路，不使用科大讯飞。中国电信云南昆明访问自建服务主页约 90 ms，热连接接口约 20–95 ms，短句翻译约 614 ms，翻译加完整 Piper 音频约 3.3 秒。右 Alt 四通道聚合已在其他前台窗口下验证；战地实际焦点与反作弊环境仍需用户实测。详见 [免费自建服务指南](SELF_HOSTED_API.md)。
